...
export const getUserOrders = async (
  ...
  try {
    ...
    const orders = await Order.find({ user: userId })
      .select('_id totalItems totalPrice originalTotalPrice discount status createdAt updatedAt items collectionItems')
      .populate({
        path: 'items.product',
...
export const placeOrder = async (
  ...
  try {
    ...
    const orderCollectionItems = populatedCollectionItems.map(collectionItem => {
      console.log('Processing collection:', collectionItem.collectionId.name);
      console.log('Collection base price:', collectionItem.collectionId.price);
      ...
      const collectionTotalPrice = collectionUnitPrice * collectionItem.quantity; // $20 * 1 = $20
      const originalTotalPrice = individualProductsTotal * collectionItem.quantity; // What it would cost individually
      const collectionDiscount = originalTotalPrice - collectionTotalPrice; // Savings from bundle

      console.log(`Collection unit price: $${collectionUnitPrice}`);
      console.log(`Individual products would cost: $${individualProductsTotal}`);
      console.log(`Collection total (${collectionItem.quantity}x): $${collectionTotalPrice}`);
      console.log(`Original total: $${originalTotalPrice}`);
      console.log(`Collection discount: $${collectionDiscount}`);
    ...
    orderCollectionItems.forEach(collectionItem => {
      orderTotalPrice += collectionItem.totalPrice;        // Add $20 (collection price)
      orderOriginalTotalPrice += collectionItem.originalTotalPrice; // Add $136 (individual prices)
      orderTotalItems += collectionItem.quantity; // Collections count as 1 item per quantity
    });

    orderTotalDiscount = orderOriginalTotalPrice - orderTotalPrice;

    console.log('Final order calculations:');
    console.log(`Total Price: $${orderTotalPrice}`);        // Should be $20
    console.log(`Original Total: $${orderOriginalTotalPrice}`); // Should be $136
    console.log(`Discount: $${orderTotalDiscount}`);        // Should be $116
    console.log(`Total Items: ${orderTotalItems}`);
    ...
    const newOrder = new Order({
      ...
      totalPrice: orderTotalPrice,              // $20
      originalTotalPrice: orderOriginalTotalPrice, // $136
      discount: orderTotalDiscount,             // $116
      status: 'pending',
      createdAt: new Date(),
      updatedAt: new Date()
    });
