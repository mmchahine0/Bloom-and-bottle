import type { PerfumeResponse, PerfumeFilters, Perfume } from "./MenP.types"
import { apiClient } from "@/api/base"
import { ENDPOINTS } from "@/api/endpoints"

export const getMenPerfumes = async (filters?: PerfumeFilters): Promise<PerfumeResponse> => {
  ...
  const endpoint = params.toString()
    ? `${ENDPOINTS.Products.Perfume.GetMen}?${params.toString()}`
    : ENDPOINTS.Products.Perfume.GetMen
