  ...
  featuredItems: [
    {
      ...
      image: "", // Removed placeholder image
      type: "perfume",
    },
    {
      _id: "skeleton-2",
  ...
  feedbacks: [
    {
      ...
      screenshot: "", // Removed placeholder image
    },
    {
...
export default function HomePageContent() {
  ...
  const handleAddToCart = (collectionId: string) => {
    const collection = displayData?.collections.find(
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section
        id="main"
        className="w-full py-4 px-4 sm:px-6 lg:px-8 bg-gray-100 "
      >
      ...
      {displayData?.collections && displayData.collections.length > 0 && (
        <section
          className="container mx-auto mb-10 px-2 py-6 relative"
          style={{
            background: `url(${collectionsBackground}) center/cover no-repeat`,
          }}
        >
          <div
            ...
            style={{ zIndex: 0 }}
          ></div>
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-semibold mb-6 lg:mb-8 text-center text-white relative z-10">
            Collections
          </h2>
          <div
            className={
          ...
          >
            {displayData.collections.map((collection) => (
              <div
                key={collection._id}
                className={`min-w-[220px] max-w-xs md:max-w-sm lg:max-w-sm xl:max-w-md w-full flex-shrink-0 md:min-w-0 md:w-auto bg-white rounded-xl shadow-md border border-gray-200 p-3 md:p-4 lg:p-5 xl:p-6 flex flex-col transition hover:shadow-lg ${
                  error ? "opacity-75" : ""
                }`}
              >
                ...
                <p
                  className="mb-2 lg:mb-3 text-[11px] md:text-xs lg:text-sm xl:text-sm text-[#6d4c41] leading-tight"
                ...
                <div className="flex items-center justify-between mb-3 lg:mb-4">
                  <span
                    className="px-2 py-1 lg:px-2.5 lg:py-1 xl:px-3 xl:py-1.5 rounded-full bg-[#2c2c2c] text-white font-extrabold text-sm lg:text-sm xl:text-base tracking-wide shadow-sm font-mono"
                    style={{ fontFamily: "Fira Mono,monospace" }}
                  >
                    ${collection.price?.toFixed(2)}
                  </span>
                  {error && (
                    <span className="text-xs lg:text-xs text-gray-400 ml-2">
                      Demo
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-3 lg:gap-4 mb-4 lg:mb-5 flex-1">
                  {collection.products &&
                    collection.products
                      .slice(0, window.innerWidth >= 1024 ? 6 : 4)
                      .map((product) => (
                        <div
                          key={product._id}
                          className="bg-gray-50 rounded-lg p-2 lg:p-3 xl:p-4 flex flex-col items-center border border-gray-200 hover:bg-gray-100 transition-colors"
                        >
                          <img
                            src={product.image}
                            alt={product.name}
                ...
                <Button
                  ...
                  disabled={!!error}
                  className={`mt-auto px-4 py-2 lg:px-5 lg:py-2.5 xl:px-6 xl:py-3 rounded-full text-sm lg:text-sm xl:text-base font-medium transition bg-[#2c2c2c] text-white hover:bg-gray-800 cursor-pointer ${
                    error ? "bg-gray-400 text-gray-600 cursor-not-allowed" : ""
                  }`}
                >
