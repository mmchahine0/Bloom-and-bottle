...
import { SampleResponse, SampleFilters, Sample } from "./WomenS.types"
import { apiClient } from "@/api/base"

export const getAllWomenSamples = async (filters?: SampleFilters): Promise<SampleResponse> => {
  ...
  const endpoint = params.toString()
    ? `${ENDPOINTS.Products.Sample.GetWomen}?${params.toString()}`
    : ENDPOINTS.Products.Sample.GetWomen
...
export const getWomenSampleById = async (id: string): Promise<{ success: boolean; data: Sample }> => {
  const response = await apiClient({
    method: "GET",
